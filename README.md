...
generator emits one Go struct per message plus the `Serialize` / [`Visitor`]
pair that drives the [`Encoder`] / [`Decoder`] primitives here, wrapped in the
one-shot `Encode` / `Decode<Name>` helpers (CORELIB_PLAN §6.1.1 fixes those
names), the same way protobuf-go's generated code calls its runtime.
...
generated code still checks once at `Flush`.
...
CORELIB_PLAN §6.1.1 closes the names that layer may use — `encode` / `decode` /
`try_decode` / `serialize` / `deserialize` / `decoder`, in the language's own
casing, with no second spelling invented beside them — so `sofabgen` emits, per
message, the **streaming pair** `Serialize(*sofab.Encoder)` and the
`sofab.Visitor` methods (the deserialize half), plus the one-shot `Encode()` /
`Decode<Name>(data)` wrappers over them and their streaming twins
`EncodeTo(io.Writer)` / `Decode<Name>From(io.Reader)`. A hand-written stand-in,
...
func (m *Point) Serialize(e *sofab.Encoder) { e.WriteSigned(1, int64(m.X)); e.WriteSigned(2, int64(m.Y)) }
...
The docs are tested too: `closed_names_test.go` scans this README and every `.go`
file for a spelling of the generated-object layer that CORELIB_PLAN §6.1.1 does
not allow, so the walkthroughs above cannot drift back to a surface `sofabgen`
does not emit.
