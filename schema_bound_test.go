...
// TestPullSchemaBoundedWaivesLimit is the pull surface's half: the caller
// declares the current field schema-bounded with SchemaBounded, and the cap is
// not applied to it. Without the declaration the same bytes are ErrLimitExceeded.
func TestPullSchemaBoundedWaivesLimit(t *testing.T) {
	arr := encodeUnsignedArray(t, boundedID, []uint64{1, 2, 3, 4, 5})
	str := encodeString(t, boundedID, "hello")
	blob := encodeBlob(t, boundedID, []byte{1, 2, 3, 4, 5})

	// Declared: decodes.
	d := sofab.NewDecoder(bytes.NewReader(arr), sofab.WithMaxArrayCount(2))
	mustNext(t, d)
	d.SchemaBounded()
	got, err := sofab.ReadUnsignedArray[uint64](d)
	if err != nil {
		t.Fatalf("declared array read = %v, want nil", err)
	}
	if len(got) != 5 {
		t.Fatalf("read %d elements, want 5", len(got))
	}
	// Not declared: the cap still applies.
	d = sofab.NewDecoder(bytes.NewReader(arr), sofab.WithMaxArrayCount(2))
	mustNext(t, d)
	if _, err := sofab.ReadUnsignedArray[uint64](d); !errors.Is(err, sofab.ErrLimitExceeded) {
		t.Fatalf("undeclared array read = %v, want ErrLimitExceeded", err)
	}

	// String and blob, same shape.
	d = sofab.NewDecoder(bytes.NewReader(str), sofab.WithMaxStringLen(2))
	mustNext(t, d)
	d.SchemaBounded()
	if s, err := d.String(); err != nil || s != "hello" {
		t.Fatalf("declared string read = %q, %v, want \"hello\", nil", s, err)
	}
	d = sofab.NewDecoder(bytes.NewReader(blob), sofab.WithMaxBlobLen(2))
	mustNext(t, d)
	d.SchemaBounded()
	if b, err := d.Bytes(); err != nil || len(b) != 5 {
		t.Fatalf("declared blob read = %d bytes, %v, want 5, nil", len(b), err)
	}
	d = sofab.NewDecoder(bytes.NewReader(blob), sofab.WithMaxBlobLen(2))
	mustNext(t, d)
	if _, err := d.Bytes(); !errors.Is(err, sofab.ErrLimitExceeded) {
		t.Fatalf("undeclared blob read = %v, want ErrLimitExceeded", err)
	}
}

// TestPullSchemaBoundedIsPerField pins the lifetime of the declaration: it
// covers the field it was made for and no other, so the next Next starts capped
// again.
func TestPullSchemaBoundedIsPerField(t *testing.T) {
	var buf bytes.Buffer
	e := sofab.NewEncoder(&buf)
	if err := sofab.WriteUnsignedArray(e, boundedID, []uint64{1, 2, 3, 4, 5}); err != nil {
		t.Fatal(err)
	...
	d := sofab.NewDecoder(bytes.NewReader(buf.Bytes()), sofab.WithMaxArrayCount(2))
	mustNext(t, d)
	d.SchemaBounded()
	if _, err := sofab.ReadUnsignedArray[uint64](d); err != nil {
		t.Fatalf("bounded field = %v, want nil", err)
	}
	mustNext(t, d)
	if _, err := sofab.ReadUnsignedArray[uint64](d); !errors.Is(err, sofab.ErrLimitExceeded) {
		t.Fatalf("following field = %v, want ErrLimitExceeded (declaration is per field)", err)
	}
}
